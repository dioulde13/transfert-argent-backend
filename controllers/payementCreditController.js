...
const ajouterPayementCredit = async (req, res) => {
  try {
    ...
    if (!credit) {
      return res
        .status(404)
        .json({ message: "Crédit introuvable avec cette référence." });
    }
    if (utilisateur.solde >= montant) {
      ...
      if (montantEnCoursPayement > credit.montant) {
        ...
        return res.status(400).json({
          message: `Le montant payé (${montant.toLocaleString(
            "fr-FR"
          )} GNF) dépasse le montant restant (${montantRestant.toLocaleString(
            "fr-FR"
          )} GNF).`,
        });
      }
      ...
      if (credit.type === "ENTRE") {
        if (credit.devise === "GNF") {
          if (utilisateur.solde >= montant) {
            utilisateur.solde = (utilisateur.solde || 0) - montant;
          } else {
            return res.status(400).json({ message: "Solde insuffisant." });
          }
        } else if (credit.devise === "XOF") {
          if (utilisateur.soldePayerAvecCodeXOF >= montant) {
            utilisateur.soldePayerAvecCodeXOF =
              (utilisateur.soldePayerAvecCodeXOF || 0) - montant;
          } else {
            return res.status(400).json({ message: "Solde insuffisant." });
          }
        } else if (credit.devise === "EURO") {
          if (utilisateur.soldePayerAvecCodeEuro >= montant) {
            utilisateur.soldePayerAvecCodeEuro =
              (utilisateur.soldePayerAvecCodeEuro || 0) - montant;
          } else {
            return res.status(400).json({ message: "Solde insuffisant." });
          }
        } else if (credit.devise === "USD") {
          if (utilisateur.soldePayerAvecCodeDolar >= montant) {
            utilisateur.soldePayerAvecCodeDolar =
              (utilisateur.soldePayerAvecCodeDolar || 0) - montant;
          } else {
            return res.status(400).json({ message: "Solde insuffisant." });
          }
        }
        ...
        return res.status(400).json({
          message: "Type de paiement invalide (doit être ENTRE ou SORTIE).",
        });
      }
